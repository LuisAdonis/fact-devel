...
router.post('/', async (req, res) => {
    try {
        const doc = new Factura_1.default(req.body);
        await doc.save();
...
router.post('/complete', async (req, res) => {
    try {
        ...
        const resultado = await factura_service_1.FacturaService.crearFacturaCompleta(req.body.factura);
...
router.get('/', async (_req, res) => {
    try {
        const docs = await Factura_1.default.find();
        res.json(docs);
    }
    catch (err) {
        res.status(500).json({ message: 'Server error' });
...
router.get('/:id', async (req, res) => {
    try {
        const doc = await Factura_1.default.findById(req.params.id);
        if (!doc)
...
router.put('/:id', async (req, res) => {
    try {
        const doc = await Factura_1.default.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!doc)
            return res.status(404).json({ message: 'Not found' });
        res.json(doc);
    }
...
router.delete('/:id', async (req, res) => {
    try {
        const doc = await Factura_1.default.findByIdAndDelete(req.params.id);
        if (!doc)
            return res.status(404).json({ message: 'Not found' });
